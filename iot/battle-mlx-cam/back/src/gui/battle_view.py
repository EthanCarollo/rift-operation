...
class BattleView(ctk.CTkFrame):
    ...
    def _process(self, role, p):
        try:
            ...
            # 0. Initial Delay: Wait 5s after attack starts before doing anything (let users draw)
            if self.current_attack and (time.time() - self.attack_start_time < 5.0):
                 remaining = 5.0 - (time.time() - self.attack_start_time)
                 self.after(0, lambda: p.rec_label.configure(text=f"⏳ Drawing time... ({remaining:.1f}s)", text_color="#eab308"))
                 return

            # 1. DEMO MODE: Bypass KNN - directly use counter based on current_attack
            ...
            if target_label:
                ...
                # No attack defined (or unknown), skip processing
                self.after(0, lambda: p.rec_label.configure(text="⏳ Waiting attack...", text_color="#666"))
                return

            # 2. Cost Control: Check if 3 seconds passed since last generation
