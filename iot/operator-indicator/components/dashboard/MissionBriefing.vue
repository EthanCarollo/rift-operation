...
<span class="text-[#00FFC2]">•</span> <strong>Secteur Cauchemar</strong> : derrière la banquise en formation<br><br>`
    ...
    case 6:
      title.value = 'DIRECTIVE'
      message.value = `<strong class="text-green-400">Excellent travail !</strong><br><br>Transition vers le <strong class="text-3xl">Monde 2</strong> : jouez la musique spécifique pour faire fuir l'inconnu dans le dernier secteur dimensionnel pour le confronter`
      break

    case 7:
      title.value = 'SUCCÈS MONDE 2'
      message.value = `<strong class="text-3xl text-green-400">✓ Monde 2 sécurisé</strong><br><br>L'inconnu a été repoussé avec succès dans le dernier secteur dimensionnel. Préparez-vous pour la phase finale de la mission !`
      if (stateTimer) clearTimeout(stateTimer)
      stateTimer = setTimeout(() => {
...
<span class="text-[#00FFC2]">•</span> <strong>Secteur Cauchemar</strong> : derrière les algues du xylophone<br><br>`
    ...
    case 9:
      ...
      case 10:
        title.value = 'DIRECTIVE'
        message.value = `<strong class="text-green-400">Excellent travail !</strong><br><br>Transition vers le <strong class="text-3xl">Monde 3</strong> : affrontez l'inconnu directement pour l'affaiblir et ainsi le capturer définitivement !`
        if (stateTimer) clearTimeout(stateTimer)
        stateTimer = setTimeout(() => {
          briefingState.value = 11
        }, 15000)
        break

      case 11:
        title.value = 'SUPPORT TACTIQUE'
        message.value = `Regardez les notes laissées par les agents précédents pour savoir comment combattre l'inconnu`
        break

      case 12:
        title.value = 'ARMEMENT PRÊT'
        message.value = `Les armes dessinées par vos agents semblent pouvoir contrer l'attaque de l'inconnu.<br><br>Dessiner le <strong>bon dessin</strong>d'armes pour réussir à l'affaiblir !`
        break

      case 13:
        title.value = 'IMPACT CONFIRMÉ'
        message.value = `<strong class="text-green-400">Bravo !</strong> Le contre semble efficace !<br>Continuez comme ça pour l'affaiblir suffisamment afin de le capturer`
        if (stateTimer) clearTimeout(stateTimer)
        stateTimer = setTimeout(() => {
          briefingState.value = 11
        }, 10000)
      ...
      case 14:
        title.value = 'CIBLE AFFAIBLIE'
        message.value = `<strong class="text-3xl text-red-500 animate-pulse">ATTENTION !</strong><br><br>L'inconnu est assez affaibli pour être capturé !<br><strong>ALLEZ-Y OPÉRATEUR : PLACEZ LA CAGE SUR LA BOÎTE POUR LE CAPTURER !</strong>`
        break

      case 15:
        title.value = 'VICTOIRE'
        message.value = `<strong class="text-4xl text-green-400">FÉLICITATIONS !</strong><br><br>L'inconnu a été capturé avec succès !`
        if (stateTimer) clearTimeout(stateTimer)
        stateTimer = setTimeout(() => {
          briefingState.value = 16
        }, 10000)
        break

      case 16:
        title.value = 'LOCALISATION FINALE'
        message.value = `<strong>Derniers fragments de faille localisés :</strong><br><br>Les deux se trouvent sous la petite excroissance de la souche d'arbre`
        break

      case 17:
        title.value = 'STABILISATION FINALE'
        message.value = `La faille est en cours de fermeture définitive.<br><br><strong class="text-3xl">Opérateur</strong>, initiez la procédure de verrouillage finale !`
        break

      case 18:
        title.value = 'MISSION ACCOMPLIE'
        message.value = `<strong class="text-4xl text-purple-400">BRAVO LES AGENTS !</strong><br><br>La faille est complètement refermée ! C'est du beau boulot pour des nouveaux !<br>Agents sur le terrain, vous pouvez revenir dans le sas pour le discord du directeur et le débriefing`
        break
    }
  }
