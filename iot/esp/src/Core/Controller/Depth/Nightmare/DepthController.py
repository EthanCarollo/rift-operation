...
class DepthController(EspController):
    ...
    async def play_partition(self):
        ...
        Mode "Simon Says": 
        1. D'abord affiche toutes les notes à jouer en séquence (LEDs)
        2. Puis le joueur doit reproduire la séquence
        """
        partition = self.state.get("depth_partition", [])
        position = self.state.get("depth_partition_position", 0)
        ...
        while position < len(partition):
            ...
            for note in sequence_notes:
                ...
                await self.play_sound(self.note_to_sound.get(note, "FA"))
