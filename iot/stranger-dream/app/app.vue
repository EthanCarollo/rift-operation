<template>
  ...
  >
    ...
    <div class="container relative z-10 flex flex-col items-center justify-center min-h-screen p-8 transition-all duration-500">
      ...
      <div v-if="strangerState === 'inactive'" class="transition-opacity duration-500">
          <h1 class="text-6xl md:text-8xl text-[#4A90E2] font-black uppercase tracking-[0.2em] drop-shadow-[0_4px_8px_rgba(255,255,255,0.8)] opacity-90">
            Stranger Dream
          </h1>
      </div>
      ...
      <div v-else class="text-center animate-pulse-slow w-full max-w-2xl px-8 relative z-10 flex flex-col items-center">
        ...
        <!-- PUZZLE TYPE -->
        <div v-if="currentQuestion && currentQuestion.type === 'puzzle'" class="flex flex-col items-center w-full bg-white/60 backdrop-blur-2xl p-10 rounded-[40px] border-4 border-[#FFD700]/60 shadow-[0_20px_50px_rgba(74,144,226,0.4)]">
            <h2 class="text-[#FF1493] font-black uppercase text-3xl tracking-[0.15em] mb-4 drop-shadow-[0_2px_4px_rgba(255,255,255,1)]">{{ currentQuestion.title }}</h2>
            <p class="text-[#1E90FF] font-black text-2xl leading-tight mb-10 whitespace-pre-line text-center drop-shadow-sm">{{ currentQuestion.subtitle }}</p>
            ...
            <div class="flex flex-col gap-8 w-full max-w-sm">
                <div v-for="(item, idx) in currentQuestion.puzzleItems" :key="idx" class="flex items-center gap-8 group">
                    <div class="w-24 h-24 flex items-center justify-center bg-white/80 rounded-full group-hover:scale-110 group-hover:bg-white transition-all duration-500 shadow-md border-2 border-[#B0E0E6]/50">
                        <img :src="item.image" class="h-16 object-contain" />
                    </div>
                    <div class="text-[#FFD700] text-5xl font-black animate-bounce-x drop-shadow-[0_3px_6px_rgba(0,0,0,0.2)]">{{ item.symbol }}</div>
                    <div class="bg-white w-20 h-20 rounded-2xl flex items-center justify-center text-5xl font-black text-[#1E90FF] shadow-2xl border-b-[10px] border-[#87CEEB] group-hover:scale-110 group-hover:-rotate-3 transition-all">
                        {{ item.letter }}
                    </div>
                </div>
            </div>
        ...
        <!-- SIMPLE TEXT TYPE -->
        <div v-else-if="currentQuestion" class="bg-white/60 backdrop-blur-2xl p-12 rounded-[40px] border-4 border-[#FFD700]/60 shadow-[0_20px_50px_rgba(74,144,226,0.4)] max-w-3xl transform hover:scale-105 transition-transform duration-500">
            <h2 class="text-[#1E90FF] font-black uppercase text-4xl md:text-5xl tracking-wide text-center leading-relaxed drop-shadow-[0_2px_4px_rgba(255,255,255,0.8)]">
        ...
        <div v-if="currentQuestion" class="flex gap-12 mt-16">
            <div v-for="i in 4" :key="i" class="relative group">
                ...
                <div v-if="currentQuestion.step === i" class="relative">
                    <div class="w-16 h-16 bg-[#FFD700] rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(255,215,0,0.8)] border-4 border-white animate-bounce-slow">
...
.snow-1 { animation: snow-fall 10s linear infinite; }
.snow-2 { animation: snow-fall 15s linear infinite; background-size: 80px 80px; opacity: 0.3; filter: blur(2px); }
.snow-3 { animation: snow-fall 20s linear infinite; background-size: 150px 150px; opacity: 0.2; filter: blur(3px); }
