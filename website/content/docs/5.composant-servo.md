...
| :--- | :--- | :--- |
| **Rouge** | **Alim Externe (5V-6V)** | Source de puissance séparée |
...
## Utilisation dans LOST

Dans l'expérience LOST, le servo est contrôlé automatiquement par les états :

### État IDLE (Début de partie)
```python
# LostStateIdle.py
self.workshop.hardware.set_servo(0)  # Piège fermé
```

### État DONE (Fin de partie)
```python
# LostStateDone.py
self.workshop.hardware.set_servo(180)  # Ouvre le piège !
await asyncio.sleep_ms(2000)           # Maintient 2 secondes
```
