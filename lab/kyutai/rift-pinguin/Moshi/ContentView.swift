...
struct ContentView: View {
    ...
    @State var selectedModel: ModelSelect = .asr
    @State var sendToSpeaker = false
    @Environment(DeviceStat.self) private var deviceStat

    // Only allow ASR with local model
    private let availableModels: [ModelSelect] = [.asr]
...
class Evaluator {
    ...
    // No longer need modelDir, we use Main Bundle
    ...
    // Renamed to make it clear we are NOT downloading
    func getModelFile(filename: String) async throws -> URL {
        let mapping: [String: String] = [
             "tokenizer-dbaa9758-checkpoint125.safetensors": "mimi-pytorch-e351c8d8@125.safetensors",
             "moshi-70f8f0ea@500.q8.safetensors": "model.safetensors",
             "tokenizer_spm_8k_0.json": "tokenizer.json",
             "tokenizer_spm_32k_3.json": "tokenizer.json"
         ]
         
        let targetName = mapping[filename] ?? filename
        
        // Split name and extension
        let name: String
        let ext: String?
        if let lastDot = targetName.lastIndex(of: ".") {
            name = String(targetName[..<lastDot])
            ext = String(targetName[targetName.index(after: lastDot)...])
        } else {
            name = targetName
            ext = nil
        }
        ...
        if let url = Bundle.main.url(forResource: name, withExtension: ext) {
            print("Found bundle resource: \(url.path)")
            return url
        }
        
        // Also check if directory logic is preferred by user or if files are in subdirectory
        // Some setups might have a "kyutai-model" folder added as a folder reference
        if let folderUrl = Bundle.main.url(forResource: "kyutai-model", withExtension: nil) {
            let fileUrl = folderUrl.appendingPathComponent(targetName)
             if FileManager.default.fileExists(atPath: fileUrl.path) {
                 return fileUrl
            }
        }

        throw CustomError("File not found in Bundle: \(filename) (mapped to \(targetName))")
    }
    ...
    func loadVocab(_ cfg: LmConfig) async throws -> [Int: String] {
        ...
        let fileURL = try await getModelFile(filename: filename)
    ...
    func makeMimi(numCodebooks: Int) async throws -> Mimi {
        ...
        let url = try await getModelFile(filename: "tokenizer-dbaa9758-checkpoint125.safetensors")
...
struct MimiModel: Model {
    ...
    init(_ ev: Evaluator, _ cb: Callbacks) async throws {
        ...
        let codeURL = try await ev.getModelFile(filename: "bria-codes.safetensors")
...
struct QwenModel_: Model {
    ...
    init(_ ev: Evaluator, _ cb: Callbacks) async throws {
        ...
        let configUrl = try await ev.getModelFile(filename: "config.json")
        let configData = try Data(contentsOf: configUrl)
        let decoder = JSONDecoder()
        decoder.keyDecodingStrategy = .convertFromSnakeCase
        let cfg = try decoder.decode(QwenConfig.self, from: configData)

        let url = try await ev.getModelFile(filename: "model.safetensors")
...
struct HeliumModel: Model {
    ...
    init(_ ev: Evaluator, _ cb: Callbacks) async throws {
        ...
        let url = try await ev.getModelFile(filename: "helium-1-preview-2b-q4.safetensors")
...
struct AsrModel: Model {
    ...
    init(_ ev: Evaluator, _ cb: Callbacks) async throws {
        await ev.setModelInfo("building model")
        let url: URL
        url = try await ev.getModelFile(filename: "moshi-70f8f0ea@500.q8.safetensors")
...
struct MoshiModel: Model {
    ...
    init(_ ev: Evaluator, _ cb: Callbacks) async throws {
        ...
        let cfg = LmConfig.moshi1b(audioDelay: 2)
            url = try await ev.getModelFile(filename: "moshi-37c6cfd6@200.q6.safetensors")
